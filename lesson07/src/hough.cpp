...
cv::Mat buildHough(cv::Mat sobel) {// единственный аргумент - это результат свертки Собелем изначальной картинки
    ...
    int max_r = hypot(height, width); // TODO замените это число так как вам кажется правильным (отталкиваясь от разрешения картинки - ее ширины и высоты)
    ...
    for (int y0 = 0; y0 < height; ++y0) {
        for (int x0 = 0; x0 < width; ++x0) {
            ...
            for (int theta0 = 0; theta0 < max_theta; ++theta0) {
                ...
                float r0 = x0*cos(theta0*PI/180) + y0*sin(theta0*PI/180);
                ...
                if(j<0) {
                    accumulator.at<float>(-j, abs(180 - i)) += strength;
                    continue;
                }
