...
void test1PerElementProcessing() {
    ...
    std::cout << "  Naive version:  " << (t1 = t.elapsed()) << " s" << std::endl; // выводим в консоль замер времени (в секундах)

    t.restart(); // перезапускаем таймер
    perElementProcessingOMP(data2); // TODO сделайте многопоточную версию поэлементного умножения
    std::cout << "  OpenMP version: " << (t2 = t.elapsed()) << " s" << std::endl;
...
void test2TotalSum() {
...

    {
        ...
        #pragma omp parallel
        {
            ...
            #pragma omp critical
            {
                cout<<"Thread #" << (id = ++n) << " started..." << std::endl;
            };
...
void test4HowWorkloadIsBalanced() {
    int n = 100 * 1000 * 1000;
    std::vector<int> data = generateNElements(n);
    vector<int> cnt(8);
    int ind = 0;
    #pragma omp parallel
    {
